...
{	
	...
	newnode->one_word.unique_word= malloc(strlen(word)+1);
	strlcpy( newnode->one_word.unique_word, word,strlen(word)+1);
	newnode->one_word.word_count=1;
...
{  
	if (p_list!=NULL && word!=NULL && strlen(word)!=0){
		...
		struct node *new_head=create_node (word);
		new_head->p_next=p_list->p_head;
		if (p_list->p_head==NULL){
			p_list->p_head=new_head;
			p_list->p_tail=p_list->p_head;
			p_list->p_current=p_list->p_head;
		}
		else{
			new_head->p_next=p_list->p_head;
			p_list->p_head->p_previous=new_head;
			p_list->p_head=new_head;
			p_list->p_current=p_list->p_head;
