...
int test_student()
{
	...
	assert(result==3,"\"day\" Expected word count to be 3 but got%d",result)
	  ? passcount++ : failcount++;
	...
	word = get_prev_word(&list1);
        assert( strcmp(word.unique_word,"will") == 0, "word next to last word in the list is \"will\", but got \"%s\"", word.unique_word )
                ? passcount++ : failcount++ ;
